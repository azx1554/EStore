 ...
 * rights reserved.
 */
package com.yang.backgroud.common.persistence.dialect.db;

import com.yang.backgroud.common.persistence.dialect.Dialect;
...
public class SQLServer2005Dialect implements Dialect {
	...
	private String getLimitString(String querySqlString, int offset, int limit, String limitPlaceholder) {
		...
		result.append("WITH query AS (SELECT ").append(distinctStr).append("TOP 100 PERCENT ").append(" ROW_NUMBER() OVER (").append(orderby).append(") as __row_number__, ").append(pagingBuilder)
				.append(") SELECT * FROM query WHERE __row_number__ BETWEEN ").append(offset).append(" AND ").append(offset + limit).append(" ORDER BY __row_number__");
